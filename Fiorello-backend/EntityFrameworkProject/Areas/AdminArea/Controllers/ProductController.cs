...
namespace EntityFrameworkProject.Areas.AdminArea.Controllers
{
    ...
    public class ProductController : Controller
    {
        ...
        //    ProductListVM product = await _context.Products.Where(m=>!m.IsDeleted).ToListAsync(id);
        ...
        public async Task<IActionResult> Detail(int? id)
        {
            ...
            ProductDetailVM productDetail = new ProductDetailVM
            {
                Title = product.Title,
                Description = product.Description,
                Image = product.ProductImages.Where(m => !m.IsDeleted).FirstOrDefault()?.Image,
                CategoryName = product.Category.Name,
                Price = product.Price
            };
        ...
        public async Task<IActionResult> Edit(int? id, ProductEditVM product)
        {
            ...
            //    Title = product.Title,
            //    Description = product.Description,
            //    //Photos = product.ProductImages.Where(m => !m.IsDeleted),
            //    CategoryName = product.Category.Name.FirstOrDefault(),
            //    Price = product.Price,
            //};
            ...
            try
            {
                ...
                if (dbProduct.Title.ToLower().Trim() == productEdit.Title.ToLower().Trim()
                    && dbProduct.Description.ToLower().Trim() == productEdit.Description.ToLower().Trim()
                    && dbProduct.Price == productEdit.Price
                    && dbProduct.CreateDate == productEdit.CreateDate
                    && dbProduct.CategoryId == productEdit.CategoryId
                    && dbProduct.ProductImages == productEdit.ProductImages
                    )
                {
                    return RedirectToAction(nameof(Index));
                }

                dbProduct.Title = productEdit.Title;
                dbProduct.Description = productEdit.Description;
                dbProduct.Price = productEdit.Price;
                dbProduct.CreateDate = productEdit.CreateDate;
                dbProduct.CategoryId = productEdit.CategoryId;
                dbProduct.ProductImages = productEdit.ProductImages;
